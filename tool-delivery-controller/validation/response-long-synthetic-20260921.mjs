...
const report = { scenario: 'synthetic long post-write completion', firstChunkMs: null, elapsedMs: null, finish: null, text: '' };
try {
  ...
  const messages = [
    ...
    llm.createAssistantMessage({ source: { provider: 'doubao', model: 'deepseek-v4-1-flash', replayState: { response: { kind: 'pi-ai', version: 2, api: 'openai-completions', provider: 'doubao', model: 'deepseek-v4-1-flash', stopReason: 'toolUse' }, blocks: [{ type: 'reasoning', thinkingSignature: 'reasoning_content' }, { type: 'tool-call' }] } }, content: [{ type: 'reasoning', text: 'The example page will contain hello. This is synthetic diagnostic data. '.repeat(Number(process.env.DSH_REASONING_REPEATS ?? 720)) }, { type: 'tool-call', id: 'fixture_write', name: 'write', arguments: JSON.stringify({ file_path: 'fixture.html', content: '<!doctype html><html><head><title>hello</title></head><body>hello<!--' + 'synthetic payload '.repeat(780) + '--></body></html>' }) }] }),
    llm.createToolResultMessage({ callId: 'fixture_write', content: [{ type: 'text', text: 'Created file fixture.html' }], isError: false }),
  ];
  for await (const chunk of ctx.llm.stream({ provider: 'doubao', model: 'deepseek-v4-1-flash', reasoningEffort: process.env.DSH_REASONING ?? 'low', maxTokens: 32768,
...
report.reasoningRepeats = Number(process.env.DSH_REASONING_REPEATS ?? 720);
report.reasoningEffort = process.env.DSH_REASONING ?? 'low';
await writeFile(new URL(`./response-long-synthetic-${report.reasoningEffort}-${report.reasoningRepeats}-20260921.json`, import.meta.url), JSON.stringify(report, null, 2));
