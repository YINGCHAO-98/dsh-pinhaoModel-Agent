...
export function toolPolicy(workspaces, routes) {
  const rootTools = new Set(['delivery_context', 'delivery_start', 'delivery_resume', 'delivery_cancel', 'delivery_status', 'capability_status',
    'todo_write', 'ask_user_question', 'multimodel_run', ...routes]);
  const executionTools = new Set([...workerTools, 'read_image', 'snapshot_explore']);
  return exec => {
    const agent = exec.agent;
    const child = Boolean(agent?.session.header.parentSession);
    if (!child && exec.name === 'ask_user_question') {
      try { workspaces.assertRootQuestionAllowed(agent.session.id); }
    ...
    if (!child && rootFileTools.includes(exec.name)) return;
    ...
    if (rootTools.has(exec.name)) return child ? 'CHILD_ORCHESTRATION_DENIED: only the root agent may orchestrate tasks' : undefined;
